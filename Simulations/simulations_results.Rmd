...
for (T_val in T) {
  for (n_val in n) {
    ...
    for (DGP_type in DGPs) {
      # Run the doMonteCarlo function for the current combination
      results <- doMonteCarlo(nsim_val, T_val, n_val, beta, DGP_type, endogenous = FALSE, error = "hetero_individual", rho = 0.5, error.type = 2)
...
for (T_val in T) { 
  for (n_val in n) {
    ...
    for (DGP_type in DGPs) {
      # Run the doMonteCarlo function for the current combination
      results <- doMonteCarlo(nsim_val, T_val, n_val, beta, DGP_type, endogenous = FALSE, error = "hetero_both", rho = 0.5, error.type = 6)
...
for (DGP in DGPs){
  all_means <- calculate_means_for_paths_and_estimators(all_results_hetero_time, DGP)
  generate_combined_latex_table(all_means, "hetero_time", DGP)
}
...
for (DGP in DGPs){
  all_means <- calculate_means_for_paths_and_estimators(all_results_hetero_individual, DGP)
  generate_combined_latex_table(all_means, "hetero_individual", DGP)
}
```
...
for (DGP in DGPs){
  all_means <- calculate_means_for_paths_and_estimators(all_results_hetero_both, DGP)
  generate_combined_latex_table(all_means, "hetero_both", DGP)
}
```
