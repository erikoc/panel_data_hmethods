...
doMonteCarlo <- function(nsim, T, n, beta, DGP = "K3", endogenous = FALSE, error = "homo") {
  ...
  exported_functions <- c("generate_X", "shift_mean", 
                          ...
                          "DataGeneratingFunction", "calculate_MSE_effects")
  ...
  sim_results <- foreach(i = 1:nsim,
    ...
    KSS_results$mse.coeff <- (KSS_results$b1.bias^2 + KSS_results$sd1^2 + KSS_results$b2.bias^2 + KSS_results$sd2^2) / 2
    ...
    Eup_results$mse.coeff <- (Eup_results$b1.bias^2 + Eup_results$sd1^2 + Eup_results$b2.bias^2 + Eup_results$sd2^2) / 2
    ...
    Within_results$mse.coeff <- (Within_results$b1.bias^2 + Within_results$sd1^2 + Within_results$b2.bias^2 + Within_results$sd2^2) / 2
